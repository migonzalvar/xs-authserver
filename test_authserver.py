import json
import os
import sqlite3
import unittest
import tempfile

from werkzeug.datastructures import Headers

import authserver


FIXTURES = {
    'OLPC_XS_DB': """
CREATE TABLE "laptops" (
    serial VARCHAR(20) NOT NULL,
    nickname VARCHAR(200) NOT NULL,
    full_name VARCHAR(100) NOT NULL,
    pubkey TEXT NOT NULL,
    uuid VARCHAR(100),
    lastmodified TEXT DEFAULT '1970-11-12 12:34:56',
    class_group INTEGER,
    PRIMARY KEY (serial)
);
INSERT INTO "laptops" VALUES('SHC23800059','miguel','','','74EDF4DF-9E81-FF4F-F053-2174AACBFB86','2013-09-19 11:14:34',3);
""",
    'DATABASE': """
CREATE TABLE users (
    username,
    pkey_hash
);
INSERT INTO "users" VALUES('fulano','bc040eb5294c5fe63f5cfd28d6961c7db6b9a2bc');
"""
}

class AuthserverTestCase(unittest.TestCase):

    def setUp(self):
        self.db_fds = {}
        for db, sql in FIXTURES.iteritems():
            fd, filename = tempfile.mkstemp()
            conn = sqlite3.connect(filename)
            conn.executescript(sql)
            conn.close()
            self.db_fds[db], authserver.app.config[db] = fd, filename
        authserver.app.config['TESTING'] = True
        self.app = authserver.app.test_client()

    def tearDown(self):
        for fd in self.db_fds.values():
            os.close(fd)
        os.unlink(authserver.app.config['DATABASE'])
        os.unlink(authserver.app.config['OLPC_XS_DB'])

    def test_index(self):
        rv = self.app.get('/')
        assert "Please login" in rv.data
        assert "pkey_hash: None" in rv.data

    def test_index_xo(self):
        pkey_hash = '195ed98f4975ebccb1e699d8636278b64e9276b3'
        cookie_value = json.dumps(dict(pkey_hash=pkey_hash))
        cookie = "xoid={cookie_value}".format(cookie_value=cookie_value)
        headers = Headers({'Cookie': cookie})
        rv = self.app.get('/', headers=headers)
        assert "pkey_hash: 195ed98f4975ebccb1e699d8636278b64e9276b3" in rv.data


if __name__ == '__main__':
    unittest.main()